...
export class LocalRelayPubSubAdapter implements RelayPubSubAdapter {
    private handlers = new Map<string, Set<(envelope: RelayPubSubEnvelope) => void>>();
    ...
    private appendRetained(topic: string, entry: RetainedPubSubEntry) {
        ...
        while (retained.entries.length - retained.head > this.retainEnvelopesPerTopic) {
            retained.head += 1;
        }
    ...
    publish(topic: string, envelope: RelayPubSubEnvelope) {
        ...
        if (events.length > 0) {
            ...
            this.appendRetained(topic, { envelope, minSeq: headSeq, maxSeq: headSeq });
        }
    ...
    subscribe(topic: string, handler: (envelope: RelayPubSubEnvelope) => void, options: RelayPubSubSubscribeOptions = {}) {
        const handlers = this.handlers.get(topic) ?? new Set<(envelope: RelayPubSubEnvelope) => void>();
        handlers.add(handler);
        this.handlers.set(topic, handlers);
        if (options.afterSeq !== undefined) {
            const retained = this.retained.get(topic);
            let replayed = 0;
            const replayLimit = options.replayLimit ?? (retained ? retained.entries.length - retained.head : 0);
            if (retained) {
                for (let index = retained.head; index < retained.entries.length && replayed < replayLimit; index += 1) {
...
function isStoreClosedError(error: unknown) {
    ...
    return e?.code === "LEVEL_DATABASE_NOT_OPEN" ||
        e?.code === "LEVEL_DATABASE_NOT_CLOSED" ||
        (typeof e?.message === "string" && /database is not open|database is closed/i.test(e.message));
}
...
function eventMatchesRequestedChannels(state: GuildState, event: GuildEvent, channels?: ChannelId[], channelSet?: Set<ChannelId>) {
    ...
    const channelId = eventChannelId(state, event);
    return !channelId || (hasChannelSet ? channelSet.has(channelId) : channels!.includes(channelId));
}
...
export class RelayServer {
    ...
    private guildAllChannelSubscriptions = new Map<GuildId, Map<WebSocket, Set<string>>>();
    private guildChannelSubscriptions = new Map<GuildId, Map<ChannelId, Map<WebSocket, Set<string>>>>();
    ...
    private pendingPubSubDeliveryEvents = new Map<string, { event: GuildEvent; observedAt: number }>();
    ...
    constructor(
        ...
        this.verboseLogging = options.verboseLogging ?? process.env.CGP_RELAY_VERBOSE_LOGS === "1";
        this.maxSocketBufferedBytes = options.maxSocketBufferedBytes ?? positiveIntegerFromEnv("CGP_RELAY_MAX_SOCKET_BUFFERED_BYTES", 4 * 1024 * 1024);
        this.maxQueuedFramesPerSocket = options.maxQueuedFramesPerSocket ?? positiveIntegerFromEnv("CGP_RELAY_MAX_QUEUED_FRAMES_PER_SOCKET", 256);
        this.maxStateCacheGuilds = options.maxStateCacheGuilds ?? positiveIntegerFromEnv("CGP_RELAY_MAX_STATE_CACHE_GUILDS", 1000);
        this.maxCachedMessageRefs = options.maxCachedMessageRefs ?? positiveIntegerFromEnv("CGP_RELAY_MAX_CACHED_MESSAGE_REFS", this.checkpointMessageRefLimit);
        this.maxPublishBatchSize = options.maxPublishBatchSize ?? positiveIntegerFromEnv("CGP_RELAY_MAX_PUBLISH_BATCH_SIZE", 1024);
        this.maxRecentPublishAcks = positiveIntegerFromEnv("CGP_RELAY_RECENT_PUBLISH_ACKS", 250_000);
        this.recentPublishAckTtlMs = positiveIntegerFromEnv("CGP_RELAY_RECENT_PUBLISH_ACK_TTL_MS", 60 * 60 * 1000);
        this.maxPubSubBatchEvents = options.maxPubSubBatchEvents ?? positiveIntegerFromEnv("CGP_RELAY_PUBSUB_BATCH_EVENTS", 512);
        this.maxFanoutDrainSocketsPerTick = options.maxFanoutDrainSocketsPerTick ?? positiveIntegerFromEnv("CGP_RELAY_FANOUT_DRAIN_SOCKETS_PER_TICK", 2048);
        this.fanoutDrainDelayMs = options.fanoutDrainDelayMs ?? nonNegativeIntegerFromEnv("CGP_RELAY_FANOUT_DRAIN_DELAY_MS", 0);
        this.pubSubReplayLimit = positiveIntegerFromEnv("CGP_RELAY_PUBSUB_REPLAY_LIMIT", 100000);
        this.relayInstanceId = options.instanceId || `${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
        this.pubSubAdapter = options.pubSubAdapter;
        this.peerUrls = options.peerUrls ?? parsePeerUrls(process.env.CGP_RELAY_PEERS);
        this.peerCatchupIntervalMs = options.peerCatchupIntervalMs ?? positiveIntegerFromEnv("CGP_RELAY_PEER_CATCHUP_INTERVAL_MS", 0);
        const defaultPeerCatchupQuorum = this.peerUrls.length <= 1 ? 1 : 2;
        this.peerCatchupMinCanonicalHeads = options.peerCatchupMinCanonicalHeads
            ?? positiveIntegerFromEnv("CGP_RELAY_PEER_CATCHUP_MIN_CANONICAL_HEADS", defaultPeerCatchupQuorum);
        this.peerCatchupNoSourceCooldownMs = options.peerCatchupNoSourceCooldownMs
            ?? positiveIntegerFromEnv("CGP_RELAY_PEER_CATCHUP_NO_SOURCE_COOLDOWN_MS", 1000);
        this.trustedPeerReadPublicKeys = new Set(options.trustedPeerReadPublicKeys ?? parsePublicKeyList(process.env.CGP_RELAY_TRUSTED_PEER_READ_PUBLIC_KEYS));
        this.wireFormat = options.wireFormat ?? wireFormatFromValue(process.env.CGP_RELAY_WIRE_FORMAT) ?? "json";
        this.pubSubLiveTopics = options.pubSubLiveTopics ?? process.env.CGP_RELAY_PUBSUB_LIVE_TOPICS !== "0";
        ...
        this.wss = new WebSocketServer({ server: this.httpServer, perMessageDeflate: false });

        const configuredPrivateKey = relayPrivateKeyFromHex(options.relayPrivateKeyHex ?? process.env.CGP_RELAY_PRIVATE_KEY_HEX);
            ...
            socket.on("message", (data) => {
                ...
                const nextTask = currentQueue.then(async () => {
                    ...
                    try {
                        ...
                        const { kind, payload } = parseCgpWireData(data, { includeRawFrame: false });
                        ...
                        if (e instanceof InvalidCgpFrameError) {
                            this.sendFrame(socket, "ERROR", { code: "INVALID_FRAME", message: "Parse error" });
                        } else {
                            console.error("Error handling message", e);
                            this.sendFrame(socket, "ERROR", { code: "INTERNAL_ERROR", message: "Internal relay error" });
                        }
                    }
                });
        ...
        const parsedCheckpointInterval = rawCheckpointInterval === undefined ? NaN : Number(rawCheckpointInterval);
        const checkpointIntervalMs = rawCheckpointInterval === undefined
            ? 60000
            : Number.isFinite(parsedCheckpointInterval) && parsedCheckpointInterval >= 0
    ...
    private async handleOpsHttp(req: IncomingMessage, res: ServerResponse, pathname: string) {
        ...
        const ready = !this.closing && this.httpServer.listening;
        const payload = {
    ...
    private renderPrometheusMetrics() {
        const lines: string[] = [];
        const metric = (name: string, value: number, help: string, type = "gauge") => {
            lines.push(`# HELP ${name} ${help}`);
            lines.push(`# TYPE ${name} ${type}`);
            lines.push(`${name} ${Number.isFinite(value) ? value : 0}`);
        };
        metric("cgp_relay_uptime_seconds", (Date.now() - this.startedAt) / 1000, "Relay process uptime in seconds.");
        metric("cgp_relay_websocket_clients", this.wss.clients.size, "Currently connected websocket clients.");
        metric("cgp_relay_subscriptions", this.countSubscriptions(), "Active client subscriptions.");
        metric("cgp_relay_fanout_queued_frames", this.countQueuedFanoutFrames(), "Queued fanout frames waiting for socket delivery.");
        metric("cgp_relay_pubsub_topics", this.pubSubUnsubscribers.size, "Active relay pubsub topic subscriptions.");
        metric("cgp_relay_pubsub_pending_guilds", this.pubSubPendingGuilds.size, "Guilds waiting for pubsub subscription setup.");
        metric("cgp_relay_hosted_guilds", this.hostedGuilds.size, "Guilds this relay has hosted or replicated.");
        metric("cgp_relay_state_cache_guilds", this.stateCache.size, "Guild states in the relay state cache.");
        metric("cgp_relay_observed_head_guilds", this.observedRelayHeads.size, "Guilds with observed relay-head gossip.");
        metric("cgp_relay_head_conflict_guilds", this.relayHeadConflicts.size, "Guilds with relay-head conflict evidence.");
        metric("cgp_relay_event_loop_delay_mean_seconds", this.eventLoopDelay.mean / 1e9, "Mean event loop delay in seconds.");
        metric("cgp_relay_event_loop_delay_p99_seconds", this.eventLoopDelay.percentile(99) / 1e9, "P99 event loop delay in seconds.");
        return `${lines.join("\n")}\n`;
    }
    ...
    private async handleMessage(socket: WebSocket, kind: string, payload: unknown) {
        ...
        for (const plugin of this.plugins) {
            try {
                ...
                this.sendFrame(socket, "ERROR", { code: "PLUGIN_ERROR", message: "Plugin frame handler failed" });
                return;
            }
        }
        ...
        if (kind === "HELLO") {
            ...
            if (plugin) {
                try {
                    ...
                    this.sendFrame(socket, "PLUGIN_CONFIG_OK", { pluginName: p.pluginName });
                } catch (e: any) {
                    console.error(`Relay plugin ${p.pluginName} configuration failed:`, e);
                    this.sendFrame(socket, "ERROR", { code: "PLUGIN_CONFIG_ERROR", message: "Plugin configuration failed" });
                }
            } else {
                this.sendFrame(socket, "ERROR", { code: "PLUGIN_NOT_FOUND", message: `Plugin ${p.pluginName} not found` });
            }
        } else if (kind === "SUB") {
            const p = objectPayload(payload) as { subId?: string; guildId?: GuildId; channels?: ChannelId[] } & SignedReadRequest;
            ...
            const p = objectPayload(payload) as { batchId?: string; events?: PublishPayload[] };
            ...
            if (eventCount === 0) {
                this.sendRequestError(socket, "VALIDATION_FAILED", "PUBLISH_BATCH requires events", { batchId });
                return;
            }
            ...
            this.sendFrame(socket, "PUB_BATCH_ACK", { batchId, results, truncated: inputEvents.length > eventCount });
            ...
            await this.withGuildMutex(guildId, async () => {
                ...
                for (const plugin of this.plugins) {
                    try {
                        ...
                        if (members) {
                            this.sendFrame(socket, "MEMBERS", { subId, guildId, members, nextCursor: null, hasMore: false });
                            return;
                        }
                    } catch (e: any) {
            ...
            this.sendFrame(socket, "ERROR", { code: "UNKNOWN_FRAME", message: `Unsupported frame kind: ${kind}` });
        }
    }
    ...
    private async rebuildGuildState(guildId: GuildId): Promise<{ state: GuildState; endEvent: GuildEvent; checkpointEvent?: GuildEvent } | null> {
        ...
        if (cachedState?.headSeq === endEvent.seq) {
            ...
            this.cacheGuildState(guildId, cachedState, this.checkpointCache.get(guildId));
            return {
                state: cachedState,
                endEvent,
    ...
    private async signRelayHead(
        ...
        const checkpointEvent = rebuilt?.checkpointEvent ?? this.checkpointCache.get(guildId);
    ...
    private recordRelayHeadObservation(head: RelayHead): RelayHeadQuorum | undefined {
        if (!verifyRelayHead(head)) {
            return undefined;
        }
        const guildId = head.guildId;
        const observations = this.observedRelayHeads.get(guildId) ?? new Map<string, RelayHead>();
        ...
        if (previous && previous.headSeq === head.headSeq && previous.headHash !== head.headHash) {
            heads.push(previous);
        }
    ...
    private async observedHeadsForGuild(guildId: GuildId, ownHead?: RelayHead | null) {
        ...
        if (selfHead) {
            ...
            const existingIndex = heads.findIndex((head) => this.relayHeadObservationKey(head) === key);
            if (existingIndex >= 0) {
    ...
    private async publishRelayHeadGossip(guildId: GuildId) {
        ...
        void Promise.resolve(this.pubSubAdapter.publish(this.pubSubHeadTopic(guildId), {
            originId: this.relayInstanceId,
            guildId,
            head,
            frame
        })).catch((e) => {
            console.error(`Relay head gossip publish failed for guild ${guildId}:`, e);
        });
    }
    ...
    private async pubSubReplayOptions(guildId: GuildId): Promise<RelayPubSubSubscribeOptions> {
    ...
    private async requestPubSubLogReplay(guildId: GuildId, targetSeq?: number) {
        ...
        const handler = (envelope: RelayPubSubEnvelope) => this.handlePubSubReplicationEnvelope(guildId, envelope);
        ...
        try {
            unsubscribe = await Promise.resolve(this.pubSubAdapter.subscribe(topic, handler, {
                afterSeq,
                replayLimit: this.pubSubReplayLimit
            }));
        } catch (e) {
        ...
        const finish = () => {
            ...
            if (unsubscribe) {
                void Promise.resolve(unsubscribe()).catch((e) => {
                    console.error(`Relay pubsub replay unsubscribe failed for guild ${guildId}:`, e);
                });
            }
        };
        let checking = false;
        const timer = setInterval(() => {
            ...
            void (async () => {
                try {
                    ...
                    const currentSeq = (await this.store.getLastEvent(guildId))?.seq ?? -1;
                    if (currentSeq > lastObservedSeq) {
                        lastObservedSeq = currentSeq;
                        idleSince = Date.now();
                    }
                    const reachedTarget = typeof targetSeq === "number" && currentSeq >= targetSeq;
                    ...
                    console.error(`Relay pubsub catch-up monitor failed for guild ${guildId}:`, error);
                    clearInterval(timer);
                    finish();
                } finally {
                    checking = false;
    ...
    private async requestPeerLogRange(
        ...
        afterSeq: number,
        limit: number
    ): Promise<{ events: GuildEvent[]; hasMore: boolean; nextAfterSeq: number; endSeq?: number; endHash?: string | null }> {
        return await new Promise((resolve, reject) => {
            ...
            socket.once("open", async () => {
                try {
                    const payload = await this.signedRelayReadPayload("GET_LOG_RANGE", { subId, guildId, afterSeq, limit });
            ...
            socket.on("message", (raw) => {
                try {
                    ...
                    socket.close();
                    if (kind === "ERROR") {
                        reject(new Error(payload?.code || payload?.message || "Peer log range request failed"));
                        return;
                    }
                    if (kind !== "LOG_RANGE") {
                    ...
                    const events = Array.isArray(payload.events) ? payload.events as GuildEvent[] : [];
                    const nextAfterSeq = Number.isFinite(payload.nextAfterSeq) ? Number(payload.nextAfterSeq) : afterSeq;
                    const endSeq = Number.isFinite(payload.endSeq) ? Number(payload.endSeq) : undefined;
                    const endHash = typeof payload.endHash === "string" ? payload.endHash : payload.endHash === null ? null : undefined;
                    resolve({
                        events,
    ...
    private async requestPeerRelayHead(peerUrl: string, guildId: GuildId): Promise<{ peerUrl: string; head?: RelayHead; error?: string }> {
        return await new Promise((resolve) => {
            ...
            socket.once("open", async () => {
                try {
                    const payload = await this.signedRelayReadPayload("GET_HEAD", { subId, guildId });
            ...
            socket.once("error", (error) => {
                clearTimeout(timeout);
                socket.close();
                resolve({ peerUrl, error: error instanceof Error ? error.message : String(error) });
            });
            socket.on("message", (raw) => {
                try {
                    const { kind, payload } = parseCgpWireData(raw, { includeRawFrame: false }) as { kind: string; payload: any };
                    ...
                    if (kind === "RELAY_HEAD" && payload.head) {
                        ...
                        resolve({ peerUrl, error: payload?.code || payload?.message || "peer head request failed" });
                    } else {
    ...
    private async peerCanonicalCatchupSources(guildId: GuildId) {
        ...
        const peerHeadResults = await Promise.all(this.peerUrls.map((peerUrl) => this.requestPeerRelayHead(peerUrl, guildId)));
        ...
        const heads = [
            ...(ownHead ? [ownHead] : []),
            ...peerHeadResults.flatMap((result) => result.head ? [result.head] : [])
        ];
        if (heads.length === 0) {
            return [];
        }

        const minCanonicalCount = Math.min(this.peerCatchupMinCanonicalHeads, heads.length);
        const summarized = summarizeRelayHeadQuorum(guildId, heads);
        if (summarized.conflicts.length > 0) {
            if (this.verboseLogging) {
                console.warn(`Relay peer catch-up refused conflicting heads for guild ${guildId}`);
            }
            return [];
        }
        ...
        try {
            ...
            if (this.verboseLogging) {
                console.warn(`Relay peer catch-up quorum failed for guild ${guildId}: ${error?.message || error}`);
            }
        }

        const aheadSources = peerHeadResults
            .filter((result) => result.head && (!ownHead || result.head.headSeq > ownHead.headSeq))
            .sort((left, right) => (right.head?.headSeq ?? -1) - (left.head?.headSeq ?? -1))
        ...
        if (canonical && (!ownHead || canonical.seq > ownHead.headSeq)) {
            return peerHeadResults
                .filter((result) => result.head && result.head.headSeq === canonical.seq && result.head.headHash === canonical.hash)
                .map((result) => ({ peerUrl: result.peerUrl, head: result.head! }));
        }
    ...
    private async requestPeerLogCatchup(guildId: GuildId, targetSeq?: number) {
        ...
        try {
            const pageLimit = Math.min(Math.max(this.maxPublishBatchSize * 4, 1000), 5000);
            let progressed = true;
            let rounds = 0;
            while (!this.closing && progressed && rounds < 64) {
                ...
                if (sources.length === 0) {
                    if (!madeProgress) {
                        this.peerCatchupNoSourceUntil.set(guildId, Date.now() + this.peerCatchupNoSourceCooldownMs);
                    }
                    return;
                }
                for (const source of sources) {
                    ...
                    const currentHead = await Promise.resolve(this.store.getLastEvent(guildId)).catch((error: unknown) => {
                        if (this.closing || isStoreClosedError(error)) {
                            return undefined;
                        }
                        throw error;
                    ...
                    let page: { events: GuildEvent[]; hasMore: boolean; nextAfterSeq: number; endSeq?: number; endHash?: string | null };
                    try {
                        page = await this.requestPeerLogRange(source.peerUrl, guildId, afterSeq, pageLimit);
                    } catch {
                        continue;
                    }
                    if (
                        ...
                        if (this.verboseLogging) {
                            console.warn(`Ignoring peer catch-up range whose end hash does not match signed head for guild ${guildId}`);
                        }
                        continue;
                    }
                    ...
                    const beforeSeq = (await Promise.resolve(this.store.getLastEvent(guildId)).catch((error: unknown) => {
                        if (this.closing || isStoreClosedError(error)) {
                            return undefined;
                        }
                        throw error;
                    ...
                    const accepted = await this.replicatePubSubEvents(guildId, page.events);
                    const afterAcceptedSeq = (await Promise.resolve(this.store.getLastEvent(guildId)).catch((error: unknown) => {
                        if (this.closing || isStoreClosedError(error)) {
                            return undefined;
                        }
                        throw error;
                    ...
                    progressed = progressed || accepted.length > 0 || afterAcceptedSeq > beforeSeq;
                    madeProgress = madeProgress || accepted.length > 0 || afterAcceptedSeq > beforeSeq;
                    if (madeProgress) {
    ...
    private async runPeriodicPeerCatchup() {
        if (this.closing || this.peerUrls.length === 0 || this.hostedGuilds.size === 0) {
            return;
        }
    ...
    private deliverPubSubEvents(guildId: GuildId, deliverable: GuildEvent[], sourceLength: number, frame?: string) {
    ...
    private async handlePubSubEnvelope(guildId: GuildId, envelope: RelayPubSubEnvelope) {
        if (envelope.originId === this.relayInstanceId || envelope.guildId !== guildId) {
            return;
        }
        const events = Array.isArray(envelope.events) && envelope.events.length > 0
            ? envelope.events
        ...
        if (this.pubSubLiveTopics) {
            const deliverable = this.filterRecentKnownUndeliveredPubSubEvents(envelope.guildId, events);
            const pending = deliverable.length === events.length
                ? []
            ...
            this.deliverPubSubEvents(envelope.guildId, deliverable, events.length, envelope.frame);
        } else if (envelope.event) {
            await this.replicatePubSubEvents(envelope.guildId, [envelope.event]);
            const deliverable = this.filterRecentKnownUndeliveredPubSubEvents(envelope.guildId, [envelope.event]);
            this.deliverPubSubEvents(envelope.guildId, deliverable, 1, envelope.frame);
        } else {
    ...
    private async replicatePubSubEvents(guildId: GuildId, events: GuildEvent[]) {
        ...
        return await this.withGuildMutex(guildId, async () => {
            ...
            for (const event of ordered) {
                ...
                if (this.verboseLogging) {
                    console.log(`Relay pubsub replicate guild ${guildId}: got seq ${event.seq}, expected ${expectedSeq}`);
                }
                if (event.seq !== expectedSeq || event.prevHash !== expectedPrevHash) {
                    if (this.verboseLogging) {
                        console.warn(`Skipping pubsub replication gap for guild ${guildId}: got seq ${event.seq}, expected ${expectedSeq}`);
                    }
                    void this.requestPubSubLogReplay(guildId, event.seq).catch((e) => {
                        console.error(`Relay pubsub gap catch-up failed for guild ${guildId}:`, e);
                    });
                    void this.requestPeerLogCatchup(guildId, event.seq).catch((e) => {
                        console.error(`Relay peer gap catch-up failed for guild ${guildId}:`, e);
                    });
                    break;
                }
                ...
                if (expectedId !== event.id) {
                    console.warn(`Skipping pubsub replication with invalid event id for guild ${guildId} seq ${event.seq}`);
                    continue;
                }

                if (!verifyObject(event.author, { body: event.body, author: event.author, createdAt: event.createdAt }, event.signature)) {
                    console.warn(`Skipping pubsub replication with invalid signature for guild ${guildId} seq ${event.seq}`);
                    continue;
                }
                ...
                if (event.seq > 0) {
                    ...
                    try {
                        if (this.canFastValidateOpenMessage(state, event)) {
                            ...
                            const validationState = await this.hydrateValidationIndexes(state, event);
                        ...
                        console.warn(`Skipping pubsub replication validation failure for guild ${guildId} seq ${event.seq}: ${e?.message || e}`);
                        continue;
                    }
                } else {
            ...
            if (accepted.length > 0) {
                if (this.verboseLogging) {
                    console.log(`Relay accepted pubsub replication for guild ${guildId}: ${accepted[0]?.seq}-${accepted[accepted.length - 1]?.seq}`);
                }
    ...
    private handlePubSubReplicationEnvelope(guildId: GuildId, envelope: RelayPubSubEnvelope) {
        if (envelope.originId === this.relayInstanceId || envelope.guildId !== guildId) {
            return;
        }
        const events = Array.isArray(envelope.events) && envelope.events.length > 0
        ...
        if (this.verboseLogging && events.length > 0) {
            console.log(`Relay received pubsub log replay for guild ${guildId}: ${events[0]?.seq}-${events[events.length - 1]?.seq}`);
        }
        void this.replicatePubSubEvents(envelope.guildId, events).then((accepted) => {
            if (accepted.length === 0) {
                return;
            }
            const hasPartitionedLiveDelivery = this.pubSubLiveTopics && envelope.liveTopics !== false;
            const deliverable = hasPartitionedLiveDelivery
                ? this.flushPendingPubSubDeliveries(envelope.guildId, accepted)
                : accepted;
            this.deliverPubSubEvents(envelope.guildId, deliverable, events.length, envelope.frame);
        }).catch((e) => {
            console.error(`Relay pubsub log replication failed for guild ${guildId}:`, e);
        });
    }

    private handlePubSubHeadEnvelope(guildId: GuildId, envelope: RelayPubSubEnvelope) {
        if (envelope.originId === this.relayInstanceId || envelope.guildId !== guildId || !envelope.head) {
            return;
        }
        const quorum = this.recordRelayHeadObservation(envelope.head);
        if (!quorum) {
            if (this.verboseLogging) {
                console.warn(`Ignoring invalid relay head gossip for guild ${guildId} from ${envelope.originId}`);
            }
            return;
        }
        this.deliverRelayHeadGossip(guildId, envelope.head, quorum);
        void this.requestPubSubLogReplay(guildId, envelope.head.headSeq).catch((e) => {
            console.error(`Relay pubsub head catch-up failed for guild ${guildId}:`, e);
        });
        void this.requestPeerLogCatchup(guildId, envelope.head.headSeq).catch((e) => {
            console.error(`Relay peer head catch-up failed for guild ${guildId}:`, e);
        });
    }
    ...
    private ensurePubSubTopicSubscription(
        ...
        if (!this.pubSubAdapter || this.pubSubUnsubscribers.has(topic) || this.pubSubPendingGuilds.has(topic)) {
            return;
        }
        ...
        void (async () => {
            const subscribeOptions = options.replicationOnly
                ...
                : options.headOnly
                    ...
                    : undefined;
            return await Promise.resolve(this.pubSubAdapter!.subscribe(topic, (envelope) => {
            if (options.replicationOnly) {
                ...
                void this.handlePubSubEnvelope(guildId, envelope).catch((e) => {
                    console.error(`Relay pubsub envelope handling failed for topic ${topic}:`, e);
                });
            }
            }, subscribeOptions));
        })().then((unsubscribe) => {
    ...
    private maybeReleasePubSubTopicSubscription(topic: string, hasInterest: () => boolean) {
        if (hasInterest()) {
            return;
        }
    ...
    private ensurePubSubGuildSubscription(guildId: GuildId) {
        if (this.pubSubLiveTopics) {
            this.ensurePubSubTopicSubscription(this.pubSubTopic(guildId), guildId, () => this.guildSubscriptions.has(guildId));
        } else {
            this.maybeReleasePubSubTopicSubscription(this.pubSubTopic(guildId), () => false);
        }
    ...
    private maybeReleasePubSubGuildSubscription(guildId: GuildId) {
        this.maybeReleasePubSubTopicSubscription(this.pubSubTopic(guildId), () => this.guildSubscriptions.has(guildId));
    ...
    private hasRelayHeadInterest(guildId: GuildId) {
        return this.hostedGuilds.has(guildId) || this.guildSubscriptions.has(guildId);
    }
    ...
    private hasSpecificChannelPubSubInterest(guildId: GuildId, channelId: ChannelId) {
        if (this.hasAllChannelPubSubInterest(guildId)) {
            return false;
        }
        return Boolean(this.guildChannelSubscriptions.get(guildId)?.get(channelId)?.size);
    }
    ...
    private ensurePubSubChannelSubscription(guildId: GuildId, channelId: ChannelId) {
        this.ensurePubSubTopicSubscription(
    ...
    private maybeReleasePubSubChannelSubscription(guildId: GuildId, channelId: ChannelId) {
        this.maybeReleasePubSubTopicSubscription(
            this.pubSubChannelTopic(guildId, channelId),
            () => this.hasSpecificChannelPubSubInterest(guildId, channelId)
        );
    ...
    private refreshPubSubChannelSubscriptionsForGuild(guildId: GuildId) {
        if (!this.pubSubLiveTopics) {
            ...
            for (const channelId of this.guildChannelSubscriptions.get(guildId)?.keys() ?? []) {
    ...
    private addSubscription(socket: WebSocket, subId: string, subscription: Subscription) {
        ...
        if (Array.isArray(subscription.channels) && subscription.channels.length > 0) {
        ...
        if (Array.isArray(subscription.channels) && subscription.channels.length > 0) {
            let channelSocketsByGuild = this.guildChannelSubscriptions.get(subscription.guildId);
            if (!channelSocketsByGuild) {
                channelSocketsByGuild = new Map<ChannelId, Map<WebSocket, Set<string>>>();
                this.guildChannelSubscriptions.set(subscription.guildId, channelSocketsByGuild);
            }
            for (const channelId of subscription.channels) {
                ...
                const channelSocketSubs = channelSockets.get(socket) ?? new Set<string>();
                channelSocketSubs.add(subId);
                channelSockets.set(socket, channelSocketSubs);
            }
        } else {
            let allChannelSockets = this.guildAllChannelSubscriptions.get(subscription.guildId);
            if (!allChannelSockets) {
                allChannelSockets = new Map<WebSocket, Set<string>>();
                this.guildAllChannelSubscriptions.set(subscription.guildId, allChannelSockets);
            }
            const allChannelSocketSubs = allChannelSockets.get(socket) ?? new Set<string>();
    ...
    private subscriptionFingerprint(subscription: Subscription) {
        const channels = Array.isArray(subscription.channels) && subscription.channels.length > 0
            ? [...subscription.channels].sort().join(",")
            : "*";
        return `${subscription.author ?? ""}|${channels}`;
    }

    private removeSubscriptionIndex(socket: WebSocket, subId: string, subscription: Subscription) {
    ...
    private async readAuthorOrError(socket: WebSocket, kind: string, payload: unknown, subId?: string, guildId?: string) {
        try {
            ...
            this.sendRequestError(socket, "AUTH_FAILED", this.clientSafeError("AUTH_FAILED"), { subId, guildId });
            return null;
        }
    }
    ...
    private async signedRelayReadPayload(kind: string, payload: Record<string, unknown>) {
        ...
        return {
            ...unsignedPayload,
            signature: await sign(this.keyPair.privateKey, hashObject({ kind, payload: unsignedPayload }))
        };
    }
    ...
    private async handleRelayHeadRequest(socket: WebSocket, payload: unknown) {
        ...
        if (!guildId.trim()) {
            this.sendRequestError(socket, "VALIDATION_FAILED", "GET_HEAD requires a guildId", { subId });
            return;
        }
        ...
        if (this.canReadAsPeerRelay(verifiedAuthor)) {
            const head = await this.signRelayHead(guildId);
            if (!head) {
                this.sendRequestError(socket, "NOT_FOUND", "Guild head not found", { subId, guildId });
                return;
            }
        ...
        await this.withGuildMutex(guildId, async () => {
            ...
            if (!this.canReadGuildForRequest(rebuilt.state, verifiedAuthor)) {
                this.sendRequestError(socket, "FORBIDDEN", "You do not have permission to read this guild head", { subId, guildId });
                return;
            }
    ...
    private async handleRelayHeadsRequest(socket: WebSocket, payload: unknown) {
        ...
        if (!guildId.trim()) {
            this.sendRequestError(socket, "VALIDATION_FAILED", "GET_HEADS requires a guildId", { subId });
            return;
        }
        ...
        if (this.canReadAsPeerRelay(verifiedAuthor)) {
            const ownHead = await this.signRelayHead(guildId);
            if (!ownHead) {
                this.sendRequestError(socket, "NOT_FOUND", "Guild heads not found", { subId, guildId });
                return;
            }
            this.recordRelayHeadObservation(ownHead);
        ...
        await this.withGuildMutex(guildId, async () => {
            ...
            if (!this.canReadGuildForRequest(rebuilt.state, verifiedAuthor)) {
                this.sendRequestError(socket, "FORBIDDEN", "You do not have permission to read this guild's relay heads", { subId, guildId });
                return;
            }
    ...
    private async handleStateRequest(socket: WebSocket, payload: unknown) {
        ...
        if (!guildId.trim()) {
            this.sendFrame(socket, "ERROR", { code: "VALIDATION_FAILED", message: "GET_STATE requires a guildId" });
            return;
        }
        ...
        await this.withGuildMutex(guildId, async () => {
            ...
            const rebuilt = await this.rebuildGuildState(guildId);
            if (!rebuilt) {
                this.sendFrame(socket, "ERROR", { code: "NOT_FOUND", message: "Guild state not found", subId, guildId });
                return;
            }
    ...
    private async handleHistoryRequest(socket: WebSocket, payload: unknown) {
        ...
        if (!guildId.trim()) {
            this.sendFrame(socket, "ERROR", { code: "VALIDATION_FAILED", message: "GET_HISTORY requires a guildId" });
            return;
        }
        ...
        const cachedState = this.stateCache.get(guildId);
        if (this.store.getHistory && this.canServeOpenChannelHistoryFast(cachedState, headEvent, author, query)) {
            const events = await this.store.getHistory(query);
            this.sendHistorySnapshot(socket, subId, guildId, query.channelId, events, limit, query.afterSeq, headEvent);
            return;
        }

        const rebuilt = await this.rebuildGuildState(guildId);
        if (!rebuilt) {
            this.sendFrame(socket, "ERROR", { code: "NOT_FOUND", message: "Guild history not found", subId, guildId });
            return;
        }
    ...
    private canServeOpenChannelHistoryFast(
        ...
        if (!state || !headEvent || state.headSeq !== headEvent.seq || !query.channelId || query.includeStructural) {
            return false;
        }
        ...
        return !Array.isArray(channel.permissionOverwrites) || channel.permissionOverwrites.length === 0;
    }
    ...
    private sendHistorySnapshot(
        ...
        const pageEvents = afterSeq !== undefined
        ...
        const tailEvent = pageEvents.length > 0 ? pageEvents[pageEvents.length - 1] : fallbackTailEvent;
    ...
    private async handleLogRangeRequest(socket: WebSocket, payload: unknown) {
        ...
        if (!guildId.trim()) {
            this.sendFrame(socket, "ERROR", { code: "VALIDATION_FAILED", message: "GET_LOG_RANGE requires a guildId", subId });
            return;
        }
        ...
        const verifiedAuthor = await this.readAuthorOrError(socket, "GET_LOG_RANGE", payload, subId, guildId);
        if (verifiedAuthor === null) return;

        if (this.canReadAsPeerRelay(verifiedAuthor)) {
            const endEvent = await this.store.getLastEvent(guildId);
            if (!endEvent) {
                this.sendRequestError(socket, "NOT_FOUND", "Guild log not found", { subId, guildId });
                return;
            }

            const rawEvents = this.store.getEventRange
                ? await this.store.getEventRange({ guildId, afterSeq, limit: limit + 1 })
                : (await this.store.getLog(guildId)).filter((event) => event.seq > afterSeq).slice(0, limit + 1);
            const hasMore = rawEvents.length > limit;
            const events = rawEvents.slice(0, limit);
            const newestSeq = events.length > 0 ? events[events.length - 1].seq : afterSeq;
        ...
        await this.withGuildMutex(guildId, async () => {
            ...
            if (!this.canReadGuildForRequest(rebuilt.state, verifiedAuthor)) {
                this.sendRequestError(socket, "FORBIDDEN", "You do not have permission to read this guild log", { subId, guildId });
                return;
            }
            ...
            const events = filterEventsForReader(rebuilt.state, rawPage, verifiedAuthor);
            const newestSeq = rawPage.length > 0 ? rawPage[rawPage.length - 1].seq : afterSeq;
            const rangeHash = hashObject({
                guildId,
                afterSeq,
                eventIds: events.map((event) => event.id)
    ...
    private async handleSearchRequest(socket: WebSocket, payload: unknown) {
        ...
        await this.withGuildMutex(guildId, async () => {
            ...
            const searchScopes = indexedMembers
                ...
                : request.scopes;
            const searchRequest = indexedMembers ? { ...request, scopes: searchScopes } : request;
            const events = searchRequest.scopes.has("messages")
                ? await this.readSearchCandidateEvents(request)
                : [];
            const built = buildSearchResults(rebuilt.state, events, author, searchRequest, indexedMembers?.results ?? []);
    ...
    private publishReplayKey(guildId: GuildId, author: string, clientEventId?: string) {
    ...
    private publishReplayPayloadHash(item: Pick<PublishPayload, "body" | "author" | "clientEventId">) {
        return hashObject({
            body: item.body,
            author: item.author,
    ...
    private rememberRecentDeliveredPubSubEvents(guildId: GuildId, events: GuildEvent[]) {
        const now = Date.now();
        for (const event of events) {
    ...
    private prunePendingPubSubDeliveryEvents(now = Date.now()) {
        ...
        const overSizeLimit = this.pendingPubSubDeliveryEvents.size > this.maxRecentPublishAcks;
        if (!overSizeLimit && now - this.pendingPubSubPruneAt < RECENT_EVENT_PRUNE_INTERVAL_MS) {
            return;
        }
    ...
    private rememberPendingPubSubDeliveries(guildId: GuildId, events: GuildEvent[]) {
        const now = Date.now();
        for (const event of events) {
            const key = this.recentEventKey(guildId, event.id);
            if (this.recentKnownEventIds.has(key) || this.recentDeliveredPubSubEventIds.has(key)) {
                continue;
            }
    ...
    private flushPendingPubSubDeliveries(guildId: GuildId, acceptedEvents: GuildEvent[]) {
        this.prunePendingPubSubDeliveryEvents();
        const deliverable: GuildEvent[] = [];
        for (const event of acceptedEvents) {
            const key = this.recentEventKey(guildId, event.id);
            if (!this.pendingPubSubDeliveryEvents.has(key) || this.recentDeliveredPubSubEventIds.has(key)) {
                continue;
            }
            this.pendingPubSubDeliveryEvents.delete(key);
    ...
    private filterRecentKnownUndeliveredPubSubEvents(guildId: GuildId, events: GuildEvent[]) {
        this.pruneRecentEventIds(this.recentKnownEventIds);
        this.pruneRecentEventIds(this.recentDeliveredPubSubEventIds);
        return events.filter((event) => {
            const key = this.recentEventKey(guildId, event.id);
            return this.recentKnownEventIds.has(key) && !this.recentDeliveredPubSubEventIds.has(key);
        });
    }
    ...
    private lookupRecentPublishAck(guildId: GuildId, item: Pick<PublishPayload, "body" | "author" | "clientEventId">) {
    ...
    private rememberRecentPublishAck(guildId: GuildId, item: Pick<PublishPayload, "body" | "author" | "clientEventId">, ack: PublishAck) {
        const key = this.publishReplayKey(guildId, item.author, item.clientEventId);
        if (!key) {
            return;
        }
    ...
    private messagePublishRefCandidate(item: Pick<PublishPayload, "body">): { messageId: string; channelId: string } | undefined {
        const body = item.body as unknown as Record<string, unknown>;
        if (body?.type !== "MESSAGE") {
            return undefined;
        }
        if (typeof body.messageId !== "string" || !body.messageId.trim() || typeof body.channelId !== "string" || !body.channelId.trim()) {
            return undefined;
        }
    ...
    private existingMessageAckFromRef(
        ...
        if (typeof ref.eventId !== "string" || !ref.eventId.trim() || !Number.isFinite(ref.seq)) {
            return undefined;
        }
    ...
    private async lookupExistingMessageAck(guildId: GuildId, item: Pick<PublishPayload, "body" | "author" | "clientEventId">): Promise<PublishAck | undefined> {
        ...
        const ref = await this.store.getMessageRef({ guildId, messageId: candidate.messageId });
        return this.existingMessageAckFromRef(guildId, item, candidate.channelId, ref);
    }

    private async lookupExistingMessageAcks(guildId: GuildId, items: IndexedPublishPayload[]): Promise<Map<number, PublishAck>> {
        const existing = new Map<number, PublishAck>();
        if (!this.store.getMessageRef && !this.store.getMessageRefs) {
            return existing;
        }

        const candidates: Array<{ item: IndexedPublishPayload; messageId: string; channelId: string }> = [];
        ...
        for (const item of items) {
            ...
            candidates.push({ item, messageId: candidate.messageId, channelId: candidate.channelId });
        ...
        if (this.store.getMessageRefs) {
            ...
            const resolved = await Promise.all(uniqueMessageIds.map(async (messageId) => {
    ...
    private async appendSequencedEventsBatch(
        ...
        const sendError = (item: IndexedPublishPayload, code: string, message: string) => {
            if (socket) {
                this.sendFrame(socket, "ERROR", {
                    code,
                    message,
        ...
        for (const item of items) {
            ...
            if (body.type === "CHECKPOINT") {
                sendError(item, "VALIDATION_FAILED", "CHECKPOINT events are relay-maintained and cannot be published by clients");
                continue;
            }

            if (typeof item.author !== "string" || !item.author.trim() || typeof item.signature !== "string" || !item.signature.trim() || !Number.isFinite(item.createdAt)) {
                sendError(item, "VALIDATION_FAILED", "Publish requires author, signature, and createdAt");
                continue;
            }

            if (!verifyObject(item.author, { body, author: item.author, createdAt: item.createdAt }, item.signature)) {
        ...
        for (const [targetGuildId, group] of grouped) {
            const groupResults = await this.withGuildMutex(targetGuildId, async () => {
                const accepted: PublishBatchResult[] = [];
                const acceptedEvents: GuildEvent[] = [];
                const newlyDurableAcks: Array<{ item: IndexedPublishPayload; ack: PublishAck }> = [];
                ...
                let checkpointEvent: GuildEvent | undefined = this.checkpointCache.get(targetGuildId);
                    ...
                    if (!state || state.headSeq !== nextSeq - 1) {
                        ...
                        if (history.length === 0) {
                            for (const item of group) {
                                sendError(item, "VALIDATION_FAILED", "Missing history for non-genesis event");
                            }
                            return accepted;
                        }
                ...
                const duplicateAcks = await this.lookupExistingMessageAcks(targetGuildId, group);
                ...
                for (const item of group) {
                    ...
                    if (!item.signatureVerified && !verifyObject(author, { body, author, createdAt }, signature)) {
                    ...
                    if (nextSeq > 0) {
                        if (!state) {
                            sendError(item, "VALIDATION_FAILED", "Missing state for non-genesis event");
                            continue;
                        }
                        ...
                        try {
                            if (this.canFastValidateOpenMessage(state, fullEvent)) {
                                ...
                                state = applyEvent(validationState, fullEvent, { mutable: true });
                            }
                            ...
                            sendError(item, "VALIDATION_FAILED", this.clientSafeValidationError(e?.message));
                            continue;
                        }
                    } else {
                        if (body.type !== "GUILD_CREATE") {
                            console.error("Validation failed: First event must be GUILD_CREATE");
                            sendError(item, "VALIDATION_FAILED", "First event must be GUILD_CREATE");
                            continue;
                        }
                ...
                if (acceptedEvents.length > 0) {
                    ...
                    for (const durable of newlyDurableAcks) {
                        this.rememberRecentPublishAck(targetGuildId, durable.item, durable.ack);
                    }
                    if (state) {
    ...
    private async appendSequencedEvent(
        ...
        const { body, author, signature, createdAt, clientEventId } = p;
        const targetGuildId = body && typeof body === "object" && !Array.isArray(body)
        ...
        return await this.withGuildMutex(targetGuildId, async () => {
            ...
            const duplicateAck = await this.lookupExistingMessageAck(targetGuildId, p);
            if (duplicateAck) {
            ...
            if (seq > 0) {
                ...
                try {
                    ...
                    console.error(`Validation failed for guild ${targetGuildId}: ${e.message}`);
                    console.error(`Event body:`, JSON.stringify(body));
                    sendError("VALIDATION_FAILED", this.clientSafeValidationError(e?.message));
                    return undefined;
                }
            } else {
                if (body.type !== "GUILD_CREATE") {
    ...
    private canFastValidateOpenMessage(state: GuildState, event: GuildEvent) {
        ...
        if (state.policies?.posting === "members" && !state.members.has(event.author)) {
            return false;
        }
        ...
        if (!channel || (Array.isArray(channel.permissionOverwrites) && channel.permissionOverwrites.length > 0)) {
            return false;
        }
    ...
    private cacheGuildState(guildId: GuildId, state: GuildState, checkpointEvent?: GuildEvent) {
        ...
        while (this.stateCache.size > this.maxStateCacheGuilds) {
            const oldestGuildId = this.stateCache.keys().next().value as GuildId | undefined;
            if (!oldestGuildId) {
                break;
            }
    ...
    private trimCachedMessageRefs(state: GuildState) {
        if (!Number.isFinite(this.maxCachedMessageRefs) || this.maxCachedMessageRefs < 0) {
            return;
        }

        while (state.messages.size > this.maxCachedMessageRefs) {
            const oldestMessageId = state.messages.keys().next().value as string | undefined;
            if (!oldestMessageId) {
                break;
            }
    ...
    private compactFanoutDrainPending() {
        ...
        if (this.fanoutDrainPendingHead < 1024 && this.fanoutDrainPendingHead * 2 < this.fanoutDrainPending.length) {
            return;
        }
        this.fanoutDrainPending = this.fanoutDrainPending.slice(this.fanoutDrainPendingHead);
        this.fanoutDrainPendingHead = 0;
    }
    ...
    private drainFanoutQueues() {
        ...
        try {
            let processed = 0;
            while (this.fanoutDrainPendingHead < this.fanoutDrainPending.length && processed < this.maxFanoutDrainSocketsPerTick) {
    ...
    private drainFanoutQueue(socket: WebSocket, queue = this.fanoutQueues.get(socket)) {
        ...
        transport?.cork?.();
        try {
            while (queue.head < queue.frames.length && socket.bufferedAmount <= this.maxSocketBufferedBytes) {
    ...
    private encodeOutboundFrame(frame: string, wireFormat: RelayWireFormat): OutboundFrame {
        return encodeCgpWireFrame(frame, wireFormat);
    }

    private encodeOutboundPayload(kind: string, payload: unknown, wireFormat: RelayWireFormat): OutboundFrame {
    ...
    private sendFanoutFrame(socket: WebSocket, frame: OutboundFrame) {
        ...
        const existingQueueLength = existingQueue ? existingQueue.frames.length - existingQueue.head : 0;
        if ((!existingQueue || existingQueueLength === 0) && socket.bufferedAmount <= this.maxSocketBufferedBytes) {
            socket.send(frame);
            return true;
        }
    ...
    private sendFanoutTextFrame(socket: WebSocket, frame: string, encodedByWireFormat?: FanoutFrameCache) {
        const wireFormat = this.socketWireFormats.get(socket) ?? this.wireFormat;
    ...
    private nonEmptySubscriptionCandidates(source: Map<WebSocket, Set<string>> | undefined) {
        return source && source.size > 0 ? source : undefined;
    }

    private candidateSubscriptionsForEvent(guildId: GuildId, state: GuildState | undefined, event: GuildEvent) {
        const channelId = state ? eventChannelId(state, event) : undefined;
        if (!channelId) {
            return this.nonEmptySubscriptionCandidates(this.guildSubscriptions.get(guildId));
        }

        const allChannelCandidates = this.nonEmptySubscriptionCandidates(this.guildAllChannelSubscriptions.get(guildId));
        const channelCandidates = this.nonEmptySubscriptionCandidates(this.guildChannelSubscriptions.get(guildId)?.get(channelId));
    ...
    private candidateSubscriptionsForEvents(guildId: GuildId, state: GuildState, events: GuildEvent[]) {
        ...
        if (channelIds.size === 1) {
            ...
            const channelCandidates = this.nonEmptySubscriptionCandidates(channelSocketsByGuild?.get(channelId));
            if (!allChannelCandidates) {
                return channelCandidates;
            }
            if (!channelCandidates) {
        ...
        for (const channelId of channelIds) {
            this.mergeSubscriptionCandidates(candidates, channelSocketsByGuild?.get(channelId));
        }
    ...
    private schedulePublicChannelSocketFanout(
        ...
        const allChannelSockets = this.guildAllChannelSubscriptions.get(guildId);
        const channelSockets = this.guildChannelSubscriptions.get(guildId)?.get(channelId);
        if ((!allChannelSockets || allChannelSockets.size === 0) && (!channelSockets || channelSockets.size === 0)) {
            return false;
        }
        this.scheduleSocketMapFanout(allChannelSockets, deliver);
        this.scheduleSocketMapFanout(channelSockets, deliver, allChannelSockets ? (socket) => allChannelSockets.has(socket) : undefined);
        return true;
    }
    ...
    private canUsePublicChannelBatchFastPath(state: GuildState, events: GuildEvent[]) {
        if (state.access === "private" || state.bans.size > 0 || events.length === 0) {
            return false;
        }
        ...
        return Boolean(channel && (!Array.isArray(channel.permissionOverwrites) || channel.permissionOverwrites.length === 0));
    }

    private publicChannelEventBatchesForFastPath(state: GuildState, events: GuildEvent[]) {
        if (state.access === "private" || state.bans.size > 0 || events.length === 0) {
            return undefined;
        }
    ...
    private canUsePublicChannelSplitBatchFastPath(guildId: GuildId, channelEvents: Map<ChannelId, GuildEvent[]>) {
        const allChannelSockets = this.guildAllChannelSubscriptions.get(guildId);
        const channelSocketsByGuild = this.guildChannelSubscriptions.get(guildId);
        if (!channelSocketsByGuild) {
            return true;
        ...
        for (const channelId of channelEvents.keys()) {
            for (const [socket, subIds] of channelSocketsByGuild.get(channelId) ?? []) {
                ...
                for (const subId of subIds) {
                    ...
                    if (!sub?.channelSet || sub.channelSet.size !== 1 || !sub.channelSet.has(channelId)) {
                        return false;
                    }
                }
    ...
    private deliverPublicChannelSplitBatch(
        ...
        if (allChannelSockets?.size) {
            this.scheduleSocketMapFanout(allChannelSockets, (socket) => {
                if (frame) {
                    ...
                    this.sendFanoutPayloadFrame(socket, "BATCH", events, fullBatchEncodedByWireFormat);
                }
            });
        }
        ...
        for (const [channelId, bucket] of channelEvents) {
            ...
            if (bucket.length === 1) {
                ...
                this.scheduleSocketMapFanout(channelSockets, (socket) => {
                    this.sendFanoutPayloadFrame(socket, "EVENT", event, encodedByWireFormat);
                }, allChannelSockets ? (socket) => allChannelSockets.has(socket) : undefined);
            } else {
                this.scheduleSocketMapFanout(channelSockets, (socket) => {
                    this.sendFanoutPayloadFrame(socket, "BATCH", bucket, encodedByWireFormat);
                }, allChannelSockets ? (socket) => allChannelSockets.has(socket) : undefined);
            }
        }
    }
    ...
    private scheduleSocketFanout<T>(items: T[], deliver: (item: T) => void) {
        ...
        let index = 0;
        const drain = () => {
            const end = Math.min(items.length, index + this.maxFanoutDrainSocketsPerTick);
    ...
    private deliverBroadcastFrame(guildId: GuildId, event: GuildEvent, frame?: string) {
        if (this.closing) {
            return;
        }
        const state = this.stateCache.get(guildId);
        const publicChannelId = state ? this.publicChannelIdForFastPath(state, event) : undefined;
        if (publicChannelId) {
            const encodedByWireFormat: FanoutFrameCache = {};
            if (!this.schedulePublicChannelSocketFanout(guildId, publicChannelId, (socket) => {
                frame
        ...
        const guildSockets = this.candidateSubscriptionsForEvent(guildId, state, event);
        if (!guildSockets) {
            return;
        }
        const encodedByWireFormat: FanoutFrameCache = {};
        ...
        for (const [socket, subIds] of guildSockets) {
            ...
            for (const subId of subIds) {
                ...
                if (state && (!eventVisibleToReader(state, event, sub.author) || !eventMatchesRequestedChannels(state, event, sub.channels, sub.channelSet))) {
                    continue;
                }
                shouldSend = true;
                break;
    ...
    private deliverBroadcastBatchFrame(guildId: GuildId, events: GuildEvent[], frame?: string) {
        if (this.closing) {
            return;
        }
        if (events.length === 0) {
        ...
        const fullBatchEncodedByWireFormat: FanoutFrameCache = {};
        const publicChannelBatches = this.publicChannelEventBatchesForFastPath(state, events);
        if (publicChannelBatches && this.canUsePublicChannelSplitBatchFastPath(guildId, publicChannelBatches)) {
            this.deliverPublicChannelSplitBatch(guildId, events, publicChannelBatches, frame);
            return;
        }

        if (this.canUsePublicChannelBatchFastPath(state, events)) {
            const channelId = eventChannelId(state, events[0]);
            if (!channelId || !this.schedulePublicChannelSocketFanout(guildId, channelId, (socket) => {
                if (frame) {
                    this.sendFanoutTextFrame(socket, frame, fullBatchEncodedByWireFormat);
                } else {
                    this.sendFanoutPayloadFrame(socket, "BATCH", events, fullBatchEncodedByWireFormat);
        ...
        const guildSockets = this.candidateSubscriptionsForEvents(guildId, state, events);
        if (!guildSockets) {
            return;
        }

        const socketGroups = new Map<string, { socket: WebSocket; subIds: Set<string>; subs: Map<string, Subscription> }[]>();
        for (const [socket, subIds] of guildSockets) {
            const subs = this.subscriptions.get(socket);
            if (!subs) {
                continue;
            ...
            for (const subId of subIds) {
                ...
                fingerprintParts.push(sub.fingerprint ?? this.subscriptionFingerprint(sub));
            }
        ...
        for (const group of socketGroups.values()) {
            ...
            for (const event of events) {
                for (const subId of first.subIds) {
                    ...
                    if (!eventVisibleToReader(state, event, sub.author) || !eventMatchesRequestedChannels(state, event, sub.channels, sub.channelSet)) {
                        continue;
                    }
            ...
            this.scheduleSocketFanout(group, ({ socket }) => {
                if (frame && outboundPayload === events) {
                    ...
                    this.sendFanoutPayloadFrame(socket, "BATCH", outboundPayload, encodedByWireFormat);
                }
            });
        }
    }

    private deliverRelayHeadGossip(guildId: GuildId, head: RelayHead, quorum: RelayHeadQuorum) {
        if (this.closing) {
            return;
        }
        const guildSockets = this.guildSubscriptions.get(guildId);
        ...
        for (const [socket, subIds] of guildSockets) {
            ...
            if (shouldSend) {
                this.sendFanoutPayloadFrame(socket, "RELAY_HEAD_GOSSIP", payload, encodedByWireFormat);
            }
        }
    }
    ...
    private publishPubSubEvents(
        ...
        if (events.length > this.maxPubSubBatchEvents) {
            for (let index = 0; index < events.length; index += this.maxPubSubBatchEvents) {
                this.publishPubSubEvents(topic, guildId, events.slice(index, index + this.maxPubSubBatchEvents), undefined, options);
            }
            return;
        }

        const envelope: RelayPubSubEnvelope = events.length === 1
            ? {
                originId: this.relayInstanceId,
                guildId,
                event: events[0]
        ...
        void Promise.resolve(this.pubSubAdapter.publish(topic, envelope)).catch((e) => {
            console.error(`Relay pubsub publish failed for topic ${topic}:`, e);
        });
    }

    private publishEventToPubSub(guildId: GuildId, event: GuildEvent, frame?: string) {
        if (!this.pubSubAdapter) {
            return;
        }

        const publishLiveTopics = this.pubSubLiveTopics && this.guildSubscriptions.has(guildId);
        this.publishPubSubEvents(this.pubSubLogTopic(guildId), guildId, [event], frame, { liveTopics: publishLiveTopics });
        ...
        const channelId = state ? eventChannelId(state, event) : undefined;
        if (!channelId) {
            this.publishPubSubEvents(this.pubSubTopic(guildId), guildId, [event], frame);
            return;
        }

        this.publishPubSubEvents(this.pubSubAllChannelsTopic(guildId), guildId, [event], frame);
        this.publishPubSubEvents(this.pubSubChannelTopic(guildId, channelId), guildId, [event], frame);
    }

    private publishBatchToPubSub(guildId: GuildId, events: GuildEvent[], frame?: string) {
        if (!this.pubSubAdapter || events.length === 0) {
            return;
        }

        const publishLiveTopics = this.pubSubLiveTopics && this.guildSubscriptions.has(guildId);
        this.publishPubSubEvents(this.pubSubLogTopic(guildId), guildId, events, frame, { liveTopics: publishLiveTopics });
        if (!publishLiveTopics) {
            return;
        }
        const state = this.stateCache.get(guildId);
        if (!state) {
            this.publishPubSubEvents(this.pubSubTopic(guildId), guildId, events, frame);
            return;
        }
        ...
        for (const [topic, topicEvents] of eventsByTopic) {
            this.publishPubSubEvents(topic, guildId, topicEvents, topicEvents.length === events.length ? frame : undefined);
        }
    }

    private broadcast(guildId: GuildId, event: GuildEvent) {
        const frame = process.env.CGP_RELAY_PUBSUB_INCLUDE_FRAME === "1"
    ...
    private scheduleBroadcastBatch(guildId: GuildId, events: GuildEvent[]) {
        ...
        this.scheduleBackgroundImmediate(() => this.broadcastBatch(guildId, events));
    }
